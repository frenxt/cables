---
title: 'Anthropic''s Responsible Scaling Policy (Sep 2023) — safety as operating procedure'
...
The RSP's contribution is not the rules themselves — it's the framing. Three words do most of the work:
...
Every AI feature we ship is assigned a PRL at spec time. The safeguards required at each tier are non-negotiable — you don't get to skip them because the deadline is tight. This is exactly the RSP's "proportional" principle, applied to production systems instead of frontier model training.
...
Before your next AI feature ships, write a one-page Production Risk Level for it. Assign the tier. List the required safeguards. Get sign-off on the list — not the product, the safeguards. The RSP's real innovation is that it separates "what we're building" from "what we're committing to have in place before we ship it." Do the same.
