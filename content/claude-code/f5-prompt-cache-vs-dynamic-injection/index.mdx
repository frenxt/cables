...
tags: ["prompt-caching", "cost-optimization", "middleware", "langgraph", "production-ai"]
difficulty: "advanced"
...
We also tried injecting dynamic content as a `HumanMessage` appended to the messages list instead of into the system prompt, hoping to preserve the system prompt cache while still getting guidance to the model. This worked for some patterns but not others — Gemini models echoed injected guidance text verbatim into their responses, treating it as content to parrot rather than instructions to internalize.
...
The code remains complete, tested, and dormant — intentionally kept for re-enablement if cost structures change or if the caching infrastructure migrates to a tier where cache misses are cheaper. The logic for detecting consecutive single writes, counting run-scoped turns, and building guidance text is all production-quality. It just doesn't run.

The remaining dynamic injections — goal context, cancellation state, Gemini communication reminders — survive because they are necessary for correct behavior and the cost of the cache miss is justified. We validated each injection against the question: "Does removing this cause observable behavioral regressions?" Goal context: yes, agent asks redundant questions. Cancellation context: yes, agent repeats cancelled work. Turn guidance: no measurable regression that exceeds the caching cost.

For the parallel execution problem specifically, we solved it differently: explicit parallel execution examples in the static system prompt and a few-shot XML example baked into the static prompt for first-message turns. Static content, cached normally.
...
- **Model-specific injection behavior is a separate constraint.** Gemini models echo injected guidance verbatim when it appears in `HumanMessage` content. This forced all behavioral guidance into the system prompt (cache-hostile) or made it purely directive without examples (less effective). The intersection of "must be in system prompt" and "must be static" is a tight constraint to design within.
...
- **Agents where correctness always wins over cost.** If every dynamic injection is truly necessary for correct behavior, accept the cache miss. Incorrect agent behavior is more expensive than cache misses.
...
After disabling turn guidance injection and keeping only correctness-critical injections, our steady-state cache hit rate returned to 90-95% on Anthropic model turns. The parallel execution anti-pattern we were trying to correct with dynamic guidance persisted at roughly the same rate as before — about 15-20% of multi-file operations were still written sequentially instead of batched. But the token cost of that inefficiency was lower than the token cost of cache-busting every affected turn. We monitor the parallel execution rate in production traces via LangSmith and plan to revisit the dynamic guidance if the inefficiency rate increases substantially or if caching costs decrease.
