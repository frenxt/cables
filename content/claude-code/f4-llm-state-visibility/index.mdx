...
tags: ["langgraph", "state-management", "context-injection", "production-ai"]
difficulty: "advanced"
...
source_links:
  - { label: "LangGraph state docs", url: "https://langchain-ai.github.io/langgraph/concepts/low_level/#state" }
---
...
We built an onboarding flow that collected a user's business name, primary goal, success metric, and design preferences into a `job_context` state field. We stored these in `AgentState` across the session. Users complained the agent kept asking clarifying questions it should already know the answers to — "What's the main goal of this project?" repeated two or three turns into a session. We dug into LangSmith traces and saw the agent had the correct values in state, but was generating those questions anyway. The model wasn't reading our state. It was making decisions from messages alone. We had built careful state management that the LLM was entirely bypassing.
</WarStory>
