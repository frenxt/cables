...
```mermaid
flowchart LR
  subgraph State["SessionState (single source of truth)"]
    Fields["typed fields: messages, plan, evals..."]
  end
  N1["entry"] -->|reads/writes state| State
  N2["coordinator"] --> State
  N3["persona nodes"] --> State
  N4["human-in-the-loop (interrupt)"] --> State
  N5["exit"] --> State
  State -. checkpointed after each node .-> DB[("Postgres checkpointer")]
  DB -. resume from last successful node .-> N1
  DB -. resume from last successful node .-> N2
  DB -. resume from last successful node .-> N3
```

Nodes never pass context through nested arguments. Everything flows through state, and state is durable. That is the whole reason the runtime exists.
...
Every node reads and writes to a shared `SessionState` schema. The checkpointer writes to Postgres after each node. A failed session, for any reason, including an LLM provider outage, resumes from the last successful checkpoint. We have had zero data loss across 100+ concurrent sessions over the engagement lifetime.
